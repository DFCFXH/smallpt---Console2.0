...
struct Camera {
    ...
    Camera(Vec pos_ = Vec(0, 0, 0), Vec dir_ = Vec(0, 0, -1), double fov_ = 45.0)
        : o(pos_), d(dir_.norm()), fov(fov_) {
        right = Vec(1, 0, 0);
        up = Vec(0, 1, 0);
    }

    Ray getRay(double x, double y, double width, double height) const {
...
Vec rotateVecY(Vec v, double angle) {
    double cosine = cos(angle);
    double sine = sin(angle);
    return Vec(v.x * cosine + v.z * sine, v.y, -v.x * sine + v.z * cosine).norm();
}
Vec rotateVecX(Vec v, double angle) {
    double cosine = cos(angle);
    double sine = sin(angle);
    return Vec(v.x, v.y * cosine - v.z * sine, v.y * sine + v.z * cosine).norm();
}
#define angle .1
#define bc 10
int main(int argc, char* argv[]) {
    ...
    Camera cam(Vec(50, 52, 295.6), Vec(0, -0.042612, -1).norm(),45); // 摄像机位置和方向
